...
function getPayloadFromTransaction(transaction) {
	...
	return (0, _solana_mobile_mobile_wallet_adapter_protocol_encoding.base64FromUint8Array)(serializedTransaction);
}
			...
			if (target[p] == null) switch (p) {
				case "signAndSendTransactions":
					target[p] = async function({ minContextSlot, commitment, skipPreflight, maxRetries, waitForCommitmentToSendNextTransaction, transactions, ...rest }) {
						...
						return base64EncodedSignatures.map(_solana_mobile_mobile_wallet_adapter_protocol_encoding.base64ToUint8Array).map(_solana_mobile_mobile_wallet_adapter_protocol_encoding.base58FromUint8Array);
					};
					break;
				case "signMessages":
					target[p] = async function({ payloads, ...rest }) {
						const base64EncodedPayloads = payloads.map(_solana_mobile_mobile_wallet_adapter_protocol_encoding.base64FromUint8Array);
						const { signed_payloads: base64EncodedSignedMessages } = await wallet.signMessages({
							...rest,
							payloads: base64EncodedPayloads
						});
						return base64EncodedSignedMessages.map(_solana_mobile_mobile_wallet_adapter_protocol_encoding.base64ToUint8Array);
					};
					break;
				case "signTransactions":
					target[p] = async function({ transactions, ...rest }) {
						...
						return base64EncodedCompiledTransactions.map(_solana_mobile_mobile_wallet_adapter_protocol_encoding.base64ToUint8Array).map(getTransactionFromWireMessage);
					};
					break;
