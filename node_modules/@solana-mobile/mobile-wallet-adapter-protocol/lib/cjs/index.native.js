...
function createSIWSMessageBase64Url(payload) {
	return require_base58Utils.encode(createSIWSMessage(payload)).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}
//#endregion
//#region src/types.ts
...
async function signInFallback(signInPayload, authorizationResult, protocolRequestHandler) {
	...
	const siwsMessage = createSIWSMessageBase64Url({
		...signInPayload,
		domain,
		address: require_base58Utils.base64ToBase58(address)
	});
	const signMessageResult = await protocolRequestHandler("sign_messages", {
		addresses: [address],
		payloads: [siwsMessage]
	});
	const signedPayload = require_base58Utils.toUint8Array$1(signMessageResult.signed_payloads[0]);
	const signedMessage = require_base58Utils.fromUint8Array$1(signedPayload.slice(0, signedPayload.length - 64));
	const signature = require_base58Utils.fromUint8Array$1(signedPayload.slice(signedPayload.length - 64));
	return {
		address,
