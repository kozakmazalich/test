...
var BaseSolanaMobileWalletAdapter = class extends _solana_wallet_adapter_base.BaseSignInMessageSignerWalletAdapter {
	...
	constructor(wallet, config) {
		...
		this.#accountSelector = async (accounts) => {
			const selectedBase64EncodedAddress = await config.addressSelector.select(accounts.map(({ publicKey }) => (0, _solana_mobile_mobile_wallet_adapter_protocol_encoding.base64FromUint8Array)(new Uint8Array(publicKey))));
			return accounts.find(({ publicKey }) => (0, _solana_mobile_mobile_wallet_adapter_protocol_encoding.base64FromUint8Array)(new Uint8Array(publicKey)) === selectedBase64EncodedAddress) ?? accounts[0];
		};
		this.#wallet = wallet;
			...
			try {
				...
				if (_solana_wallet_standard_features.SolanaSignAndSendTransaction in this.#wallet.features) {
					...
					const [signature] = (await this.#wallet.features[_solana_wallet_standard_features.SolanaSignAndSendTransaction].signAndSendTransaction({
						...
						return (0, _solana_mobile_mobile_wallet_adapter_protocol_encoding.base64FromUint8Array)(output.signature);
					});
					return signature;
				} else {
...
var RemoteSolanaMobileWalletAdapter = class extends BaseSolanaMobileWalletAdapter {
	constructor(config) {
		...
		super("remoteHostAuthority" in config ? new _solana_mobile_wallet_standard_mobile.RemoteSolanaMobileWalletAdapterWallet({
			...baseConfig,
			remoteHostAuthority: config.remoteHostAuthority
		}) : new _solana_mobile_wallet_standard_mobile.RemoteSolanaMobileWalletAdapterWallet({
			...baseConfig,
