...
async function isLocalWebSocketAvailable() {
	if (typeof navigator !== "undefined" && isSolanaMobileWebShell(navigator.userAgent)) return true;
	try {
		return (await navigator.permissions.query({ name: "loopback-network" })).state === "granted";
	} catch (e) {
		if (e instanceof TypeError && (e.message.includes("loopback-network") || e.message.includes("local-network-access"))) return true;
		return false;
	}
}
	...
	#transact = async (callback) => {
		...
		try {
			...
			const result = await Promise.race([(async () => {
				if (!useWebSocket && !this.#nostrRelay) await checkLocalNetworkAccessPermission();
			})().then(async () => {
				loadingSpinner.init();
				const { wallet, close } = !useWebSocket && this.#nostrRelay ? await (0, _solana_mobile_mobile_wallet_adapter_protocol.startNostrScenario)({
					...config,
					connectionType: "local",
					relayDomain: this.#nostrRelay
				}) : await (0, _solana_mobile_mobile_wallet_adapter_protocol.startScenario)(config);
				associating = false;
...
var RemoteSolanaMobileWalletAdapterWallet = class {
	...
	*/
	...
	constructor(config) {
		...
		this.#chains = config.chains;
		this.#chainSelector = config.chainSelector;
		this.#relay = "remoteHostAuthority" in config ? {
			kind: "reflector",
			domain: config.remoteHostAuthority
		} : {
			kind: "nostr",
			domain: config.nostrRelay
		};
		this.#onWalletNotFound = config.onWalletNotFound;
		this.#optionalFeatures = {
			[_solana_wallet_standard_features.SolanaSignAndSendTransaction]: {
	...
	#transact = async (callback) => {
		...
		try {
			modal.init();
			modal.open();
			const { associationUrl, close, wallet } = this.#relay.kind == "reflector" ? await (0, _solana_mobile_mobile_wallet_adapter_protocol.startRemoteScenario)({
				...baseConfig,
				remoteHostAuthority: this.#relay.domain
			}) : await (0, _solana_mobile_mobile_wallet_adapter_protocol.startNostrScenario)({
				...baseConfig,
...
function registerMwa(config) {
	...
	else if (getIsRemoteAssociationSupported() && ("nostrRelay" in config || config.remoteHostAuthority !== void 0)) {
		if ("nostrRelay" in config) (0, _wallet_standard_wallet.registerWallet)(new RemoteSolanaMobileWalletAdapterWallet({
			...config,
			nostrRelay: config.nostrRelay
		}));
		else (0, _wallet_standard_wallet.registerWallet)(new RemoteSolanaMobileWalletAdapterWallet({
			...config,
			remoteHostAuthority: config.remoteHostAuthority
		}));
	} else console.warn(`MWA not registered: device or environment not supported`);
}
//#endregion
