...
import { SolanaMobileWalletAdapterError, SolanaMobileWalletAdapterErrorCode, startNostrScenario, startRemoteScenario, startScenario } from "@solana-mobile/mobile-wallet-adapter-protocol";
	...
	#transact = async (callback) => {
		...
		try {
			...
			const result = await Promise.race([(async () => {
				...
				loadingSpinner.init();
				const { wallet, close } = !useWebSocket && this.#nostrRelay ? await startNostrScenario({
					...config,
	...
	*/
	...
	#transact = async (callback) => {
		...
		try {
			modal.init();
			modal.open();
			const { associationUrl, close, wallet } = this.#relay.kind == "reflector" ? await startRemoteScenario({
				...baseConfig,
...
function registerMwa(config) {
	...
	if (getIsLocalAssociationSupported() && (!isWebView(userAgent) || isSolanaMobileWebShell(userAgent))) registerWallet(new LocalSolanaMobileWalletAdapterWallet(config));
		...
		if ("nostrRelay" in config) registerWallet(new RemoteSolanaMobileWalletAdapterWallet({
			...config,
			nostrRelay: config.nostrRelay
		}));
		else registerWallet(new RemoteSolanaMobileWalletAdapterWallet({
			...config,
