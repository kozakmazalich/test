...
`).join("")}`;
  ...
  function isSuccessfulContext(returnValue) {
  ...
  function getSortedSignatoryAddresses(message) {
    return message.requiredSignatories.map(({ address: address2 }) => address2).toSorted();
  }
  function assertOffchainMessageV1Equal(receivedMessage, expectedMessage) {
    ...
    if (actualAddresses.length !== expectedAddresses.length || actualAddresses.some((address2, ii) => address2 !== expectedAddresses[ii])) {
      ...
      dispose() {
        ...
        for (let ii = cleanups.length - 1; ii >= 0; ii--) {
          try {
            ...
            error = hasError ? createSuppressedError(e3, error) : e3;
            hasError = true;
          }
        }
        cleanups.length = 0;
  ...
  var NODEJS_CUSTOM_INSPECT_SYMBOL = /* @__PURE__ */ Symbol.for("nodejs.util.inspect.custom");
  var JAVASCRIPT_PROTOCOL_PROPERTY_NAMES = /* @__PURE__ */ new Set(
    [
      "then",
      "toJSON",
  ...
  var OBJECT_PROTOTYPE_PROPERTY_NAMES = /* @__PURE__ */ new Set([
    "__defineGetter__",
    "__defineSetter__",
    "__lookupGetter__",
    "__lookupSetter__",
      ...
      async *[Symbol.asyncIterator]() {
        ...
        const onChange = () => {
          ...
          if (state.status === "loaded") {
            ...
            failure = {
              error: (_a = state.error) != null ? _a : new SolanaError(SOLANA_ERROR__SUBSCRIBABLE__STREAM_CLOSED_WITHOUT_ERROR)
            };
            wake();
          }
        };
  ...
  function toAllowedNumericMethodName(methodName) {
  ...
  function getDefaultResponseTransformerForSolanaRpcSubscriptions(config) {
    return (response, request) => {
      const methodName = toAllowedNumericMethodName(request.methodName);
      const keyPaths = (config == null ? void 0 : config.allowedNumericKeyPaths) && methodName ? config.allowedNumericKeyPaths[methodName] : void 0;
  ...
  function getApiMethodNameFromNotificationMethod(notificationMethod) {
    ...
    if (!publisher) {
      publisherByResponseTransformer.set(
        ...
        publisher = demultiplexDataPublisher(channel, "message", (rawMessage) => {
          ...
          const transformedNotification = responseTransformer ? responseTransformer(message.params.result, {
    ...
    if (!memoizedKeypaths2) {
      memoizedKeypaths2 = {
        ...
        blockNotifications: [
          ...
          ...messageConfig.map(
            (c) => ["value", "block", "transactions", KEYPATH_WILDCARD, "transaction", "message", ...c]
          ),
  ...
  function getSignersFromInstruction(instruction) {
    ...
    return deduplicateSigners(
      ((_a = instruction.accounts) != null ? _a : []).flatMap((account) => account.signer ? [account.signer] : [])
    );
  }
  function getSignersFromTransactionMessage(transaction) {
    return deduplicateSigners([
      ...
      ...transaction.instructions.flatMap((instruction) => getSignersFromInstruction(instruction))
    ]);
  }
  ...
  function decodeFromJson(rpcTx) {
    ...
    const version = (_a = rpcTx.version) != null ? _a : "legacy";
    let compiledMessage;
    switch (version) {
      case "legacy":
        compiledMessage = {
        ...
        break;
      case 0: {
        const addressTableLookups = message.addressTableLookups ? message.addressTableLookups.map((l) => ({
  ...
  // src/create-client-with-interfaces-from-rpc.ts
  function createClientWithGetMinimumBalanceFromRpc(rpc) {
    return {
      async getMinimumBalance(space, config) {
        if (config == null ? void 0 : config.withoutHeader) {
  ...
  function createClientWithFetchAccountsFromRpc(rpc) {
    return {
  ...
  function createClientWithInterfacesFromRpc(rpc) {
    return {
  ...
  exports.assertOffchainMessageV1Equal = assertOffchainMessageV1Equal;
  ...
  exports.createClientWithFetchAccountsFromRpc = createClientWithFetchAccountsFromRpc;
  exports.createClientWithGetMinimumBalanceFromRpc = createClientWithGetMinimumBalanceFromRpc;
  exports.createClientWithInterfacesFromRpc = createClientWithInterfacesFromRpc;
