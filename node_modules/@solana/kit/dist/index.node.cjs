  ...
  return {
    async getMinimumBalance(space, config) {
      if (config?.withoutHeader) {
        ...
        const lamportsPerByte = headerBalance / BigInt(accounts.BASE_ACCOUNT_SIZE);
        return rpcTypes.lamports(lamportsPerByte * BigInt(space));
      }
  ...
  return {
    async fetchAccounts(addresses, config) {
      ...
      if (addresses.length === 1) {
        return [await accounts.fetchEncodedAccount(rpc, addresses[0], config)];
      }
      return await accounts.fetchEncodedAccounts(rpc, addresses, config);
    }
  };
}
...
Object.keys(promises).forEach(function (k) {
  if (k !== 'default' && !Object.prototype.hasOwnProperty.call(exports, k)) Object.defineProperty(exports, k, {
    enumerable: true,
    get: function () { return promises[k]; }
  });
});
