...
function assertFitsHardwareWalletSignableMessageByteLimit(putativeMessage) {
  const messageBodyLength = codecsStrings.getUtf8Encoder().getSizeFromValue(putativeMessage.content.text);
...
function assertOffchainMessageV1Equal(receivedMessage, expectedMessage) {
  if (receivedMessage.content !== expectedMessage.content) {
    const utf8Encoder = codecsStrings.getUtf8Encoder();
    throw new errors.SolanaError(errors.SOLANA_ERROR__OFFCHAIN_MESSAGE__CONTENT_DOES_NOT_MATCH_EXPECTED, {
  ...
  if (actualAddresses.length !== expectedAddresses.length || actualAddresses.some((address2, ii) => address2 !== expectedAddresses[ii])) {
    throw new errors.SolanaError(errors.SOLANA_ERROR__OFFCHAIN_MESSAGE__REQUIRED_SIGNATORIES_DO_NOT_MATCH_EXPECTED, {
      actualAddresses,
      expectedAddresses
    });
  }
